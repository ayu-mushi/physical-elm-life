...
var $author$project$Main$AddLifes = function (a) {
	return {$: 'AddLifes', a: a};
};
...
var $author$project$Main$randomLife = function () {
	var vel_y = A2($elm$random$Random$float, 0, 5);
	var vel_x = A2($elm$random$Random$float, 0, 5);
	var size = 5;
	var pos_y = A2($elm$random$Random$float, 0, $author$project$Main$height);
	var pos_x = A2($elm$random$Random$float, 0, $author$project$Main$width);
	return A5(
...
var $author$project$Main$collisionUpdate = F2(
	function (life, model) {
		return function (_v0) {
			var l = _v0.a;
			var rest = _v0.b;
			return A2(
				$elm$core$List$cons,
				l,
				$elm$core$List$concat(rest));
		}(
			A3($elm_community$list_extra$List$Extra$mapAccuml, $author$project$Main$ifColDoCol, life, model.lifes));
	});
var $author$project$Main$collisionUpdateAll = function (model) {
	var new_lifes = $elm$core$List$concat(
		A2(
			...
			A2($elm$core$List$map, $author$project$Main$collisionUpdate, model.lifes)));
	return _Utils_update(
		model,
		{lifes: new_lifes});
};
...
var $author$project$Vector$fromTuple = function (_v0) {
	var x = _v0.a;
	var y = _v0.b;
	return A2($author$project$Vector$Vector2, x, y);
};
var $elm$core$Basics$ge = _Utils_ge;
var $author$project$Main$moveTo = F2(
	function (life, new_pos) {
		return (new_pos.x <= 0) ? _Utils_update(
			life,
			{
				velocity: $author$project$Vector$fromTuple(
					_Utils_Tuple2(-life.velocity.x, life.velocity.y))
			}) : ((new_pos.y <= 0) ? _Utils_update(
			life,
			{
				velocity: $author$project$Vector$fromTuple(
					_Utils_Tuple2(life.velocity.x, -life.velocity.y))
			}) : ((_Utils_cmp(new_pos.x, $author$project$Main$width) > -1) ? _Utils_update(
			life,
			{
				velocity: $author$project$Vector$fromTuple(
					_Utils_Tuple2(-life.velocity.x, life.velocity.y))
			}) : ((_Utils_cmp(new_pos.y, $author$project$Main$height) > -1) ? _Utils_update(
			life,
			{
				velocity: $author$project$Vector$fromTuple(
					_Utils_Tuple2(life.velocity.x, -life.velocity.y))
		...
		if (msg.$ === 'Frame') {
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						lifes: _Utils_ap(lifes, model.lifes)
					}),
				$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$Vector$toTuple = function (_v0) {
	var x = _v0.x;
	var y = _v0.y;
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2($elm$html$Html$Attributes$style, 'border', '2px solid rgba(0,0,0,0.1)')
							]),
						_Utils_ap(
							_List_fromArray(
								[$author$project$Main$clearScreen]),
							A2($elm$core$List$map, $author$project$Main$render, model.lifes)))
					])),
				$author$project$Main$viewCollision(model)
			]));
};
